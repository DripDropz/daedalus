...
2. **Default avatar placeholder.** Never render a broken image; always fall back to a neutral SVG. (Daedalus will only render avatars in Sprint 4 after anchor verification — but the same fallback contract applies.)
...
2. **No source labelling on rendered name/avatar.** GovTool renders the off-chain `givenName` and image directly alongside on-chain ID with no visual distinction. A naïve user can't tell what's verified vs anchor-claimed. Daedalus must label all anchor-derived content explicitly (Sprint 4). See `shared-design-tokens.md` → "Source label."
...
- The **Voting Power Calculator** ("Your ADA × current total voting power = your share %") is a powerful evaluator-side affordance. **Out of scope for Sprint 2**, but flag for Sprint 4+ consideration on detail view ("if you delegated your X ADA to this DRep, their voting power would become Y").
...
| DReps can be Ed25519 keys OR scripts (`drep_script`) | Filter facet "Type: key / script" useful in detail but low priority for Sprint 2 cards. |
...
| `image` / `imageObject` (base64 or URL+sha256) | Optional | Sprint 4 only. Render after the image-file hash is also verified. Default placeholder otherwise. |
...
| `doNotList` | Optional (default `false`) | **Sprint 4 acceptance gap.** If `true`, DRep should be excluded from default cohort but reachable via direct ID entry and search. Not currently in the plan. |
...
## Pattern Summary for Variants

All three variants in this folder share:
