    ...
    if !skip_code_sign {
        ...
        let notary_creds = match (
            ...
            (Some(apple_id), Some(password), Some(team_id)) => {
                Some(sign::NotaryCreds { apple_id, password, team_id })
            }
            _ => {
                println!("  [notarization skipped — APPLE_NOTARY_USER / APPLE_NOTARY_PASS / APPLE_TEAM_ID not set]");
                None
            }
        };
...
async fn cmd_release(
    ...
    if test {
        ...
        .await?;
        newsfeed_urls = Some((s3.cdn_url(&newsfeed_key), s3.cdn_url(&verification_key)));
    }
